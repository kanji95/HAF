...
from torchensemble import VotingClassifier, FusionClassifier, NeuralForestClassifier, BaggingClassifier, GradientBoostingClassifier, SnapshotEnsembleClassifier, AdversarialTrainingClassifier, FastGeometricClassifier, SoftGradientBoostingClassifier
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained", default=False, action='store_true', help="start from ilsvrc12/imagenet model weights")
    ...
    ensemble.set_optimizer(
        ...
        lr=opts.lr,                       # learning rate of parameter optimizer
        weight_decay=opts.weight_decay,              # weight decay of parameter optimizer
    )

    # Set the learning rate scheduler
    ensemble.set_scheduler(
        "CosineAnnealingLR",                    # type of learning rate scheduler
        T_max=opts.epochs,                           # additional arguments on the scheduler
    )

    # Train the ensemble
    ensemble.fit(
        train_loader,
        epochs=opts.epochs,                          # number of training epochs
    )
