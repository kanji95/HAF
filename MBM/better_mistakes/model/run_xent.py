...
def post_hoc_adjustment(y, a, reverse_mapping_index=None):
    """
    y: 1 x 1000
    a: 1 x 314
    """
    ...
    for i in range(a.shape[1]):
        indices = torch.tensor(reverse_mapping_index[i]).view(1, -1)
        y[torch.arange(bs)[:, None], indices] = y[torch.arange(bs)[:, None], indices] * a[torch.arange(bs), i][:, None]
    return y

def run_post_hoc(loader, model, aux_model, loss_function, distances, all_soft_labels, classes, opts, epoch, prev_steps, optimizer=None, is_inference=True, corrector=lambda x: x):
    """
        ...
        for batch_idx, (embeddings, target) in enumerate(loader):
            ...
            if (opts.arch == "custom_resnet18" or opts.arch == "wide_resnet") and opts.loss != "yolo-v2":
                if opts.loss == "ours-l12-cejsd-wtconst-dissim" or opts.loss == "ours-l7-cejsd-wtconst-dissim":
                    ...
                    aux_output = aux_model(embeddings, target, is_inference)
                else:
                    output = model(embeddings, target)
                    aux_output = aux_model(embeddings, target)
            else:
                output = model(embeddings)
                aux_output = aux_model(embeddings)
                
            if opts.post_hoc:
                output = post_hoc_adjustment(F.softmax(output, dim=-1), F.softmax(aux_output, dim=-1), reverse_mapping_index=loader.dataset.reverse_mapping_index)
