...
from MBM.better_mistakes.model.losses import HierarchicalCrossEntropyLoss, CosineLoss, RankingLoss, CosinePlusXentLoss, YOLOLoss, OurHierarchicalCrossEntropyLoss
...
def main_worker(gpus_per_node, opts):
    ...
    if opts.loss == "yolo-v2":
        ...
        # classes, _ = get_classes(hierarchy)
    else:
    ...
    if opts.optimizer == "adagrad":
        ...
        if opts.devise or opts.barzdenzler:
            if opts.data == "inaturalist19-224" or opts.data == "tiered-imagenet-224":
                ...
                return torch.optim.Adam(
                    [
                        {"params": model.features_2.parameters()},
                        {"params": model.classifier_3.parameters(), "lr": opts.lr_fc, "weight_decay": opts.weight_decay_fc},
                    ],
                    lr=opts.lr,
                    weight_decay=opts.weight_decay,
                    amsgrad=True,
            ...
            # if opts.loss == "ours-l12-cejsd-wtconst-weighted":
            #     pass
        ...
        if opts.data == "cifar-100": #or opts.loss == "soft-labels" or opts.loss == "hierarchical-cross-entropy":
            ...
            if opts.loss == "soft-labels":          # this will not be used. 
                return torch.optim.SGD([
                    {'params': model.classifier_3.parameters(), 'lr': 0.0005},          # volcanic-sweep-10 parameters.
                    {'params': model.features_2.parameters(), 'lr': 0.05},
                ], momentum=0.9, weight_decay=5e-4)
            if (opts.loss == "flamingo-l5" or opts.loss == "ours-l5-cejsd-wtconst-dissim" or opts.loss == "ours-l5-cejsd-wtconst" or opts.loss == "ours-l5-cejsd"):
                return torch.optim.SGD([
        ...
        if opts.data == "inaturalist19-224":
            ...
            if (opts.loss == "flamingo-l7" or opts.loss == "ours-l7-cejsd" or opts.loss == "ours-l7-cejsd-wtconst" or opts.loss == "ours-l7-cejsd-wtconst-dissim"):
                return torch.optim.SGD([
                    {'params': model.classifier_1.parameters(), 'lr': 0.1},
                    {'params': model.classifier_2.parameters(), 'lr': 0.1},
                    {'params': model.classifier_3.parameters(), 'lr': 0.1},
        ...
        if opts.data == "tiered-imagenet-224":
            if opts.loss == "cross-entropy" or opts.loss == "soft-labels" or opts.loss == "yolo-v2":
                return torch.optim.SGD([
                    {'params': model.classifier_3.parameters(), 'lr': 0.1},
                    {'params': model.features_1.parameters(), 'lr': 0.1},
                    {'params': model.features_2.parameters(), 'lr': 0.01},
                ], momentum=0.9, weight_decay=5e-4)
            if opts.loss == "flamingo-l12":             # Flamingo and ours doesn't work yet.
                return torch.optim.SGD([
                    {'params': model.classifier_1.parameters(), 'lr': 0.1},
                    {'params': model.classifier_2.parameters(), 'lr': 0.1},
                    {'params': model.classifier_3.parameters(), 'lr': 0.1},
